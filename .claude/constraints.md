...
**refresh token:** backend stores opaque tokens in `RTK_RefreshTokens`; `api.service.ts` transparently retries any 401 non-`skipUnauthorized` request via `POST /auth/refresh` (deduplicates concurrent refreshes); `skipUnauthorized: true` disables both the retry and the `unauthorizedHandler`.
