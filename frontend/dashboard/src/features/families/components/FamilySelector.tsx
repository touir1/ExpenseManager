  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        aria-expanded={open}
      >
        <svg className="h-3.5 w-3.5 text-ink-faint shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" />
        </svg>
        <span>{label}</span>
        <svg className={`h-3 w-3 text-ink-faint shrink-0 transition-transform duration-150 ${open ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      ...
      {open && (
        <div
          role="menu"
          className="absolute right-0 top-full mt-1 w-48 bg-surface-card border border-surface-border rounded-xl shadow-lg py-1 z-50"
        >
          <button
            role="menuitemradio"
