...
function Skeleton() {
  return (
    <div
      ...
      role="status"
      aria-label="Loading largest expenses"
    >
...
export function LargestExpenses({ data, isLoading }: Props) {
    ...
    <div className="bg-surface-card rounded-2xl border border-surface-border shadow-card p-6">
      <div className="flex items-center justify-between mb-3">
        <p className="text-xs font-semibold uppercase tracking-widest text-ink-mute">
          {t('dashboard.largest.title')}
        </p>
        <Link
          to="/expenses"
          className="text-xs font-medium text-brand-600 hover:text-brand-700 transition-colors duration-150"
        >
          {t('dashboard.largest.viewAll')} →
        </Link>
      </div>

      {data.length === 0 ? (
        <EmptyState compact icon="🏆" title={t('dashboard.largest.empty')} />
      ) : (
        <ul>
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2.5 min-w-0">
                  <span className="text-[11px] text-ink-faint shrink-0 tabular-nums w-4 text-right">
                    {index + 1}
                  </span>
