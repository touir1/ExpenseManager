...
| `POST` | `/import/preview` | Parse + validate CSV file (multipart, max 1 MB, `.csv` ext, `text/csv`/`application/csv`/`text/plain`/`application/vnd.ms-excel` MIME); rejects binary files (null-byte probe); validates required headers (`date`, `amount`, `currency_code`) and ≤20 columns; optional `columnMapping` form field (JSON `{rawHeader: canonicalField}`) remaps non-standard headers — falls back to the user's saved default mapping when omitted and headers don't match verbatim; 30s timeout → `CsvImportPreviewDto` with per-row status and error codes; `families` column accepts **semicolon-separated family names** (case-insensitive, not IDs); 400 `MISSING_HEADERS:<fields>` / `INVALID_COLUMN_MAPPING` / `TOO_MANY_COLUMNS` / `INVALID_FILE_CONTENT` on the corresponding failures |
| `POST` | `/import/detect-headers` | Read only the header row of a CSV file → `CsvHeaderDetectionDto { rawHeaders, suggestedMapping, headersMatchExactly }`; suggestions merge a built-in alias table with the user's saved default mapping; never throws `MISSING_HEADERS` |
...
| `GET` | `/config` | Authenticated user's config → `UserConfigDto { defaultCurrencyId, defaultCurrency, defaultCategoryId, defaultCsvColumnMapping }` (null fields if no row exists) |
| `PUT` | `/config` | Upsert default currency/category → `UserConfigDto`; 400 (`USER_CONFIG_INVALID_CURRENCY`) if currency/category id doesn't exist |
| `PUT` | `/config/csv-column-mapping` | Save the user's default CSV column mapping — body: `{ mapping: {rawHeader: canonicalField} }` → `UserConfigDto`; 400 (`INVALID_COLUMN_MAPPING`) if a value isn't a recognized canonical field |
| `DELETE` | `/config/csv-column-mapping` | Clear the user's saved default CSV column mapping → `UserConfigDto` |
