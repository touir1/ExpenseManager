...
- **Backend (expenses service)**: migration `AddRecurringExpenseGeneration` adds `AutoCreate bool` (default false) and `LastGeneratedDate DateOnly?` to `RecurringExpense`. `IRecurringExpenseRepository` extended: `GetPagedAsync(userId,includeInactive)`, `GetByIdAsync(id,userId)`, `AddAsync`, `UpdateAsync`, `SoftDeleteAsync(id,userId)`, `GetDueForGenerationAsync(asOfDate)` (`IsActive && !IsDeleted && NextDueDate<=asOfDate && (LastGeneratedDate==null || LastGeneratedDate<NextDueDate)`). `IRecurringExpenseService` extended: `GetAllAsync`, `GetByIdAsync`, `CreateAsync`, `UpdateAsync`, `DeleteAsync`, `ConfirmAsync(id,userId)→ExpenseDto?` (creates the real `Expense` via `IExpenseService.AddAsync`, source `SingleWeb`, advances `NextDueDate`/`LastGeneratedDate`; throws `RecurringExpenseNotDueException`→400 `RECURRING_NOT_DUE` if not yet due), `GenerateDueAsync(asOfDate,ct)` (batch job entry point, per-row try/catch so one bad template doesn't block the rest). Shared `RecurringExpenseScheduler.AdvanceNextDueDate(current,frequencyId)`: Weekly→+7d, Monthly→`AddMonths(1)`, Yearly→`AddYears(1)` (handles month-end natively, e.g. Jan 31→Feb 28/29). `RecurringExpenseController` gains full CRUD: `POST /recurring-expenses`→201 `CreatedAtRoute("GetRecurringExpenseById",...)`, `PUT/DELETE/GET /recurring-expenses/{id}`, `GET /recurring-expenses?includeInactive=false`, `POST /recurring-expenses/{id}/confirm`. `RecurringExpenseDto` extended with raw FK fields (`FrequencyId`,`IsActive`,`FamilyId`,`CategoryId`,`SubcategoryId`,`CurrencyId`,`AutoCreate`) alongside the existing display fields. New `Jobs/RecurringExpenseGenerationJob.cs` (Quartz `[DisallowConcurrentExecution]`, mirrors `RateAutoUpdateJob`), daily cron via new `RecurringExpenseOptions.GenerationTime` (env `EXPENSES_MANAGEMENT_EXPENSES_RECURRINGEXPENSE_GENERATION_TIME`, default `03:00`, after the `02:00` currency-rate job) — for each due template with `AutoCreate=true`, creates the `Expense` (source `BulkWeb`) and advances the template.
- **Frontend (`frontend/dashboard`)**: new `features/recurring-expenses/` — `recurringExpenseApi.service.ts` (`getAll,getById,create,update,remove,confirm`), `recurringExpense.schemas.ts` (RHF+zod), `RecurringExpenseForm.tsx` (mirrors `ExpenseForm.tsx`'s `AmountInput`/`FormCombobox` pattern; frequency as a 3-option select), `RecurringExpensesPage.tsx` (table of templates with add/edit/delete/pause, `EmptyState`) at new route `/recurring-expenses`, linked from `SettingsPage.tsx`'s Preferences section. `UpcomingRecurring.tsx`: due, non-`autoCreate` items get a "Confirm" button (calls `confirm(id)`, invalidates `['dashboard']` react-query keys); `autoCreate=true` items show a text "Auto" badge (no color-only meaning). New backend error codes `RECURRING_EXPENSE_NOT_FOUND`/`RECURRING_NOT_DUE` mapped in `apiErrors.constant.ts` + all 4 locale files. New `recurringExpenses.*` i18n namespace (en/fr/es/de); `dashboard.recurring.*` extended with `confirm`/`auto`/`manage`.
- Tests: backend +105 (`RecurringExpenseRepositoryTests`, `RecurringExpenseServiceTests`, `RecurringExpenseControllerTests`, new `RecurringExpenseGenerationJobTests`); frontend +43 (`recurringExpenseApi.service.test.ts`, `RecurringExpenseForm.test.tsx`, `RecurringExpensesPage.test.tsx`, `UpcomingRecurring.test.tsx` additions) — full backend (983) and frontend (1272) suites green.
